...
public:
	...
	enum MIFARE_Misc {
		...
		MF_KEY_SIZE				= MF_KEY_SIZE_CONFIG			// A Mifare Crypto1 key is 6 bytes.
	};
	...
	//CUSTOM FOR ODOTS
	bool PICC_DumpMifareClassicSectorDataToSerial(Uid *uid,MIFARE_Key *key,byte sector);
	bool PICC_DumpMifareClassicDataToSerial(	Uid *uid,PICC_Type piccType,MIFARE_Key *key);
	bool PICC_DumpDataToSerial(Uid *uid);
	
	bool PICC_CleanMifareData(Uid *uid);
	bool MFRC522::PICC_WipeOdotsData(	Uid *uid,PICC_Type piccType,MIFARE_Key *key	);
	bool MFRC522::PICC_WipeOdotsSectorData(Uid *uid,MIFARE_Key *key,byte sector);
	//END OF CUSTOM FOR ODOTS
