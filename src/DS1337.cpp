...
void InitDS1337()
	{
		...
		Wire.begin();
		i2c_write_1(DS1337_REG_CONTROL,0x00);
...
void format_mifare_write_time_str(struct time_t* time, char*buf)
{
	// Hour
	//burst three bytes of the buffer are for other use
	buf[3]=time->hour;
	//minute
	buf[4]=time->minute;
	//second
	buf[5]=time->second;
	//millisecond across two bytes (because 1000>255...)
	buf[6]=(uint8_t)(((time->millis)&0xff00)>>8);
	buf[7]=(uint8_t)((time->millis)&0x00ff);
}
